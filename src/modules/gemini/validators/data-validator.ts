...
export class DataValidator {
  ...
  static validateTeacherTitle(teacherTitle: string): void {
    ...
    if (teacherTitle.length > 100) {
      throw new HttpException(
        'Teacher title too long. Maximum 100 characters allowed.',
        HttpStatus.BAD_REQUEST,
      );
    }
  }
  ...
  static validateStudentData(studentData: unknown): StudentDataValidation {
    ...
    try {
      ...
      // If it's an array, validate each student
  ...
  static validateClassData(classData: unknown): ClassDataValidation {
    ...
    try {
      ...
      const data = classData as Record<string, any>;
  ...
  private static validateSingleStudent(
    ...
    if (student.responses && Array.isArray(student.responses)) {
      cleanedData.responses = student.responses.filter(
        (response) =>
          response &&
  ...
  private static sanitizeAnalytics(analytics: any): Record<string, any> {
    const sanitized: Record<string, any> = {};

    if (typeof analytics.classAverageScore === 'number') {
      sanitized.classAverageScore = Math.max(
        0,
        Math.min(100, analytics.classAverageScore),
      );
    }

    if (typeof analytics.totalClassTimeSpent === 'number') {
      sanitized.totalClassTimeSpent = Math.max(
        0,
        analytics.totalClassTimeSpent,
      );
    }

    if (typeof analytics.highPerformersCount === 'number') {
      sanitized.highPerformersCount = Math.max(
        0,
        analytics.highPerformersCount,
      );
    }

    if (typeof analytics.studentsNeedingHelpCount === 'number') {
      sanitized.studentsNeedingHelpCount = Math.max(
        0,
        analytics.studentsNeedingHelpCount,
      );
    }
