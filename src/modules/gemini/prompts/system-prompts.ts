...
export class SystemPrompts {
  static getStudentAnalysisPrompt(
    studentData?: unknown,
    message?: string,
  ): string {
...
- មុខវិជ្ជាដែលសិស្សពូកែបំផុត (ពិន្ទុ > ${GEMINI_CONFIG.PERFORMANCE_THRESHOLDS.EXCELLENT_SCORE_DETAILED}) ជាមួយចំនួនលំហាត់ដែលបានធ្វើ (បង្ហាញឈ្មោះមុខវិជ្ជាជាភាសាខ្មែរ)
- មុខវិជ្ជាដែលត្រូវការជំនួយបន្ទាន់ (ពិន្ទុ < ${GEMINI_CONFIG.PERFORMANCE_THRESHOLDS.BAD_SCORE}) ជាមួយពេលវេលាដែលត្រូវការ (បង្ហាញឈ្មោះមុខវិជ្ជាជាភាសាខ្មែរ)
...
- មុខវិជ្ជាដែលមានអត្រាជោគជ័យខ្ពស់បំផុត (>${GEMINI_CONFIG.PERFORMANCE_THRESHOLDS.EXCELLENT_SCORE}%)
- មុខវិជ្ជាដែលត្រូវការចំណាត់ការកាន់តែច្រើន (<${GEMINI_CONFIG.PERFORMANCE_THRESHOLDS.POOR_SCORE}%)
...
- រាយឈ្មោះសិស្សពូកែគណិត (ពិន្ទុ > ${GEMINI_CONFIG.PERFORMANCE_THRESHOLDS.EXCELLENT_SCORE_DETAILED}) ជាមួយចំនួនលំហាត់ដែលបានបញ្ចប់
- រាយឈ្មោះសិស្សដែលត្រូវការជំនួយ (ពិន្ទុ < ${GEMINI_CONFIG.PERFORMANCE_THRESHOLDS.BAD_SCORE}) ជាមួយពេលវេលាមធ្យម
- លំហាត់គណិតណាដែលមានអត្រាភាពត្រឹមត្រូវទាប (<${GEMINI_CONFIG.PERFORMANCE_THRESHOLDS.NEEDS_HELP}%)
- ប្រភេទបញ្ហាគណិតដែលពិបាកបំផុត (បូក/ដក/គុណ/ចែក)
- វិភាគកំណើនពីសប្តាហ៍មុន ឬខែមុន
- យុទ្ធសាស្ត្របង្រៀនជាក់ស្តែងសម្រាប់ការកែលម្អ`;
  }

  static getSubjectPerformancePrompt(
    subject: string,
    studentData?: unknown,
  ): string {
...
សូមបង្ហាញ:
- រាយឈ្មោះសិស្សពូកែ${subject} (ពិន្ទុ > ${GEMINI_CONFIG.PERFORMANCE_THRESHOLDS.GOOD_SCORE})
- រាយឈ្មោះសិស្សខ្សោយ${subject} (ពិន្ទុ < ${GEMINI_CONFIG.PERFORMANCE_THRESHOLDS.BAD_SCORE})
...
- លំហាត់ដែលមានអត្រាជោគជ័យខ្ពស់បំផុត (>${GEMINI_CONFIG.PERFORMANCE_THRESHOLDS.HIGH_SUCCESS_RATE}%) ជាមួយចំនួនសិស្សដែលបានធ្វើ
- លំហាត់ដែលមានអត្រាខុសច្រើនបំផុត (<${GEMINI_CONFIG.PERFORMANCE_THRESHOLDS.LOW_SUCCESS_RATE}%) ជាមួយពេលវេលាមធ្យម
...
- ${GEMINI_CONFIG.LIMITS.TOP_PERFORMERS} នាក់ដែលពូកែបំផុត (ឈ្មោះ + ពិន្ទុមធ្យម + ចំនួនលំហាត់បានធ្វើ + ពេលវេលាសរុប)
- ${GEMINI_CONFIG.LIMITS.STUDENTS_NEEDING_HELP} នាក់ដែលត្រូវការជំនួយ (ឈ្មោះ + ពិន្ទុមធ្យម + បញ្ហាចម្បង + អនុសាសន៍ជាក់ស្តែង)
