...
export class GemmaService {
  ...
  private readonly gemmaModel = 'google/gemma-3-27b-it';
  ...
  async sendMessageToGemma(
    ...
    if (!teacherTitle || teacherTitle.trim() === '') {
      throw new HttpException(
        'Teacher title cannot be null or empty',
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    const promptWithInstructions = this.buildPromptTemplate(
      message,
      teacherTitle,
      conversationContext,
    );

    // Create request body for OpenRouter API
    const requestBody = {
    ...
    try {
      const headers = {
        ...
        'HTTP-Referer': 'https://plp-edtech.com', // Replace with your app's URL
        'X-Title': 'PLP Education Platform', // Replace with your app's name
      };

      const response = await axios.post(this.openRouterApiUrl, requestBody, {
        headers,
      });
      ...
      console.error(
        'Error calling Gemma 3 API:',
        error.response?.data || error.message,
      );
      throw new HttpException(
        'Error processing your request with Gemma 3',
  ...
  private buildPromptTemplate(
    ...
    const introduction =
      'អ្នកគឺជា បញ្ញាបឋម ជំនួយការបង្រៀនគណិតវិទ្យាសម្រាប់សិស្សបឋមសិក្សាពីថ្នាក់ទី១ដល់ទី៦ នៅក្នុងប្រទេសកម្ពុជា។ អ្នកត្រូវបង្រៀនដូចគ្រូបឋមសិក្សាម្នាក់។\n\n';
    ...
    // Math topics section
    const mathTopics =
      'ផ្នែកគណិតវិទ្យាសម្រាប់បឋមសិក្សាដែលអ្នកអាចជួយបាន៖\n' +
  ...
  private replaceVietnameseWords(text: string): string {
    const replacements = {
      'phép trừ': 'ការដក',
      'phép cộng': 'ការបូក',
      'phép nhân': 'ការគុណ',
